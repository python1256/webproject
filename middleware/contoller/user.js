...
const token1=require("../model/influencer_token");
const token2=require("../model/brands_token");
const token3=require("../model/admin_token");
...
router.post("/user_login",async(req,res)=>{
    try{
        ...
        if(!ismatch){
            ...
            res.status(201).send(user_email);
        }
...
router.post("/Influencer_login",async(req,res)=>{
    try{
        ...
        if(!ismatch){
            ...
            res.status(201).send("your token:"+paru);
        }
    }catch(err){
        res.status(400).send("invalid data entry");
    }
